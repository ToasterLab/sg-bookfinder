...
lookups.users = JSON.parse(fs.readFileSync("./fbot_data.json", "utf8")) || []
...
let requestGoodreadsAuth = event => {
	bookfinder.setGoodreadsCallback("https://bookwhere.ketupat.me/fbot_callback")
	bookfinder.getSignInURL().then(url => {
		lookups.oauths[url.oauthToken] = {
			senderId: event.sender.id,
			oauthTokenSecret: url.oauthTokenSecret
		}
		bot.sendTextMessage(event.sender.id, url.url)
	}).catch(err => throwError(res,err))
}

let sendGoodreadsInfo = user => {
	bookfinder.setOAuth(user.accessToken, user.accessTokenSecret)
	bookfinder.getUserInfo(user.id).then(usr => {
		bot.sendTextMessage(user.senderId, `wait ah, ${usr.name}`)
		let bk_count = usr.shelves.filter(s => s.name === 'to-read')[0].book_count._
		bookfinder.getAllAvailableBooksOnShelf(
			usr, 'to-read', bk_count, 'BIPL', 
			interim => {}
		).then(result => {
			bot.sendQuickReply(user.senderId, canned.again.text, canned.start.buttons)
		}).catch(err => console.error(err))
	}).catch(err => {
		console.error(err)
		bot.sendTextMessage(user.senderId, "Sorry, something went wrong when accessing your Goodreads account.")
	})
}
...
let handlePayload = (event, payload) => {
	...
	switch(payload.action){
		...
		case 'goodreads':
			let exists = lookups.users.filter(v => v.senderId === event.sender.id)
			if(exists.length === 1){
				sendGoodreadsInfo(event, exists[0])
			} else {
				requestGoodreadsAuth(event)
			}
			break
	}
}
...
fbot.prototype.callback = (req, res) => {
	if(req.query.authorize === "1"){
		bookfinder.processSignIn(req.query.oauth_token, lookups.oauths[req.query.oauth_token].oauthTokenSecret, req.query.authorize).then(result => {
			res.status(200).send("Success! You can now close this window.")
			let user = {
				id: result.userid,
				senderId: lookups.oauths[req.query.oauth_token].senderId,
				accessToken: result.accessToken,
				accessTokenSecret: result.accessTokenSecret
			}
			lookups.users.push(user)
			fs.writeFileSync("./fbot_data.json", JSON.stringify(lookups.users))
			console.log(lookups.users)
			sendGoodreadsInfo(user)
		}).catch(err => {console.error(err)})
	} else {
		res.status(403).send("Oauth denied :(")
	}
}
