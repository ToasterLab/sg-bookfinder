	...
	switch(payload.action){
		...
		case 'is book available':
			if('book' in payload){
				...
				// hardcoded to BIPL first
				bot.sendTyping(event.sender.id, true)
				bookfinder.isBookAvailable({isbn: payload.book}, 'BIPL').then(results => {
					console.log(results)
					if(results.length === 0){throw Error(`${payload.book} is not available`)}
					results = results[0]
					bot.sendTextMessage(event.sender.id,
					`Yes, ${results.BranchName} got.\n${results.LocationDesc}\n${results.CallNumber}`)
				}).catch(err => {
		...
		case 'find book':
			...
			if(event.message.text){
				...
				bookfinder.findInGoodreads(event.message.text).then(result => {
					...
					console.log(result.results[0].work)
					...
					Promise.all(isbns).then(values => {
						...
						if(result.length === 1){
							let blurb = `${result[0].best_book[0].title[0]}\nby ${result[0].best_book[0].author[0].name[0]}`
							if(!('nil' in result[0].original_publication_year[0].$ &&
							result[0].original_publication_year[0].$.nil === 'true')){
								blurb += `published in ${result[0].original_publication_year[0]._}`
							}
							let btns = canned.again.buttons
							btns.unshift({
								content_type: 'text',title: 'availability',
								payload: JSON.stringify({action:'is book available', book: result[0].isbn})
							})
							bot.sendQuickReply(event.sender.id, blurb, canned.again.buttons)
						} else {
							let list = []
							result.forEach(v => {
								list.push({
