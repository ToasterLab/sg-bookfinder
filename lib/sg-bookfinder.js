...
bookfinder.prototype.processSignIn = (token, secret, url) => { // handle /callback requests
	return new Promise((resolve, reject) => {
		let params;
		try {url.parse(req.url, true)} catch (e) {reject(e)}
		gr.processCallback(token, secret, params.query.authorize, (callback) => {
			resolve(callback);
		});
	});
}
